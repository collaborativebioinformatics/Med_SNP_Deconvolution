...
## Goals 
The primary objective of this project is to develop a reproducible computational framework for privacy-preserving population stratification that enables federated learning via NVIDIA FLARE federated infrastructure ([Roth 2022](https://arxiv.org/abs/2210.13291)) across large genomic biobanks. Specifically, we aim to: (1) transform phased variant call format (VCF) data into recombination-defined haploblock clusters that serve as categorical features for machine learning, leveraging high-resolution meiotic recombination maps to define biologically meaningful genomic segments; (2) implement and benchmark GPU-accelerated training pipelines supporting both tree-based (XGBoost) and deep learning architectures optimized for the sparse nature of haploblock cluster features; (3) establish a federated learning infrastructure using NVIDIA FLARE that allows multiple sites to collaboratively train models using the Federated Averaging (FedAvg) algorithm while keeping raw genotype data localized at each 'institution'; and (4) validate the biological interpretability of learned features through alignment with known population-specific genomic regions, demonstrating that privacy-enhanced features retain meaningful biological signal.
